...
analyzer_path = ROOT / "hps" / "core" / "v25_analyzer.py"
analyzer_path.parent.mkdir(parents=True, exist_ok=True)

analyzer_path.write_text(r'''
...
# Upgrade ScriptImporter to use V25 analyzer/enrichment.
script_importer = ROOT / "hps" / "core" / "script_importer.py"
txt = script_importer.read_text(encoding="utf-8")

if "from hps.core.v25_analyzer import build_v25_plan, enrich_plan" not in txt:
    txt = txt.replace(
...
old = 'def build_plan(self, script: str, title: str, use_qwen: bool = True, model: str = "qwen2.5:7b", host: str = "http://127.0.0.1:11434"): if use_qwen: return qwen_plan(script, title, model=model, host=host)[0] return deterministic_plan(script, title)'
new = 'def build_plan(self, script: str, title: str, use_qwen: bool = True, model: str = "qwen2.5:7b", host: str = "http://127.0.0.1:11434"): plan = None\n        if use_qwen:\n            plan = qwen_plan(script, title, model=model, host=host)[0]\n        else:\n            plan = build_v25_plan(script, title)\n        return enrich_plan(plan, script)'
if old in txt:
    txt = txt.replace(old, new)
else:
    print("WARNING: ScriptImporter build_plan pattern not found. You may already have a modified version.")

script_importer.write_text(txt, encoding="utf-8")


# Upgrade wizard metrics import and summary, without touching studio_qt again.
wizard = ROOT / "hps_qt" / "dialogs" / "project_wizard.py"
w = wizard.read_text(encoding="utf-8")

if "from hps.core.v25_analyzer import plan_metrics" not in w:
    w = w.replace(
        "from hps.core.script_importer import ScriptImporter",
        "from hps.core.script_importer import ScriptImporter\nfrom hps.core.v25_analyzer import plan_metrics"
    )

# Replace _plan_stats body by delegating to V25 analyzer if possible.
start = w.find("def _plan_stats(plan: dict) -> dict:")
if start != -1:
    end = w.find("\n\n\nclass ProjectWizardDialog", start)
    if end != -1:
        replacement = '''def _plan_stats(plan: dict) -> dict:
    m = plan_metrics(plan)
    return {
        "scenes": m["scenes"],
        "blocks": m["blocks"],
        ...
        "voice_cost": m["voice_cost_estimate"],
    }
'''
        w = w[:start] + replacement + w[end:]

# Improve summary text if old section exists.
old_summary = '''f"Equipment tags: {stats['equipment']}\\n"
            f"Words: {stats['words']}\\n"
...
new_summary = '''f"Equipment tags: {stats['equipment']}\\n"
            ...
            f"Estimated voice cost only: ${stats['voice_cost']:.2f}\\n"
            f"Images/music/analysis cost: $0.00\\n\\n"'''
w = w.replace(old_summary, new_summary)

wizard.write_text(w, encoding="utf-8")


doc = ROOT / "docs" / "V25_PRODUCTION_ANALYZER.md"
doc.write_text("""# V25 Production Analyzer
...
print("V25 analyzer upgrade applied.")
print("Files changed:")
print(" - hps/core/v25_analyzer.py")
print(" - hps/core/script_importer.py")
print(" - hps_qt/dialogs/project_wizard.py")
print(" - docs/V25_PRODUCTION_ANALYZER.md")